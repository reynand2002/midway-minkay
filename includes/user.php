...
class User{
	...
	protected static $tbl_name = "useraccount";
	function db_fields(){
		global $mydb;
	...
	function single_user($id=0){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where `useraccount_id`= {$id} LIMIT 1");
	...
	function find_all_user($name=""){
	 ...
	 static function AuthenticateUser($email="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `useraccount` WHERE `username`='" . $email . "' and `password`='" . $h_upass ."' LIMIT 1");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
		 $found_user = $mydb->loadSingleResult();
		    $_SESSION['admin_id'] 	 		= $found_user->useraccount_id;
            $_SESSION['admin_name']    	= $found_user->name;
            $_SESSION['admin_username']		= $found_user->username;
            $_SESSION['admin_password']		= $found_user->password;
            $_SESSION['admin_role']    	= $found_user->role;
        	return true;
			}else{
				return false;
			}	
