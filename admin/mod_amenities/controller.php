		...
		if ($image_size == FALSE) {
			...
			if (file_exists($location)) {
				...
				if ($_POST['name'] == "" or $_POST['description'] == "") {
					...
					$amenities_name		= $_POST['name'];
					$amenities_description	= $_POST['description'];
					$amenities_image 		= $location;

					$res = $amen->find_all_amenities($amenities_name);
					...
					if ($res >= 1) {
						...
						$amen->amenities_name = $amenities_name;
						$amen->amenities_description = $amenities_description;
						$amen->amenities_image = $location;

						$istrue = $amen->create();
						if ($istrue == 1) {
							message("New [" . $amenities_name . "] created successfully!", "success");
							redirect('index.php');
						}
					}
				}
...
{
	$amen = new Amenities();
	$rm_id			= $_POST['amenities_id'];
	$rm_name		= $_POST['name'];
	$rm_description = $_POST['description'];

	$amen->amenities_id = $rm_id;
	$amen->amenities_name = $rm_name;
	$amen->amenities_description = $rm_description;
